...
export const LoginPage: React.FC = () => {
  ...
  const handleStartAuth = (providerName: string, customEmail?: string) => {
    ...
    setTimeout(() => {
      ...
      if (customEmail) setEmail(customEmail);
        ...
        setEmail(providerName === 'google' ? 'alex.investor@gmail.com' : providerName === 'apple' ? 'alex@icloud.com' : '0x4F2a...3c76');
      }
    }, 600);
  };
  ...
  const handleFinishLogin = () => {
    login(generatedAddress, email || 'alex@dopamint.ai');
    navigate('/');
  };
        ...
        <div className="flex items-center justify-center gap-2 mb-6">
          <div
            className={`px-3.5 py-1 rounded-full text-xs transition-all ${
              step === 1
                ? 'bg-[#485442]/10 dark:bg-[#55604e]/20 text-[#485442] dark:text-[#8A9E7F] font-bold border border-[#485442]/20 shadow-2xs'
