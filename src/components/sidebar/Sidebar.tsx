...
export const Sidebar: React.FC = () => {
  ...
  return (
    <div className="hidden lg:flex h-screen flex-shrink-0">
       ...
       *  Leaderboard, Favourites, Agents, Settings,
       *  Buy Credits, User Profile
       * ═══════════════════════════════════════════ */}
      <aside className="w-[260px] h-screen bg-[var(--bg-card)] border-r border-[var(--border-color)] flex flex-col justify-between p-3 flex-shrink-0">
        {/* Top — Crown + Toggle */}
          ...
          <div className="flex items-center justify-between px-1 pt-1">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <img
                src={logoDope}
                alt="dopamint"
                className="h-4.5 w-auto max-w-[120px] object-contain brightness-0 dark:brightness-100 transition-all"
              />
            </div>
            <button
              onClick={toggleSidebar}
              title={isSidebarOpen ? 'Collapse Chats' : 'Expand Chats'}
        ...
        {/* Bottom — Utility menu items + Profile */}
      ...
      {/* ═══════════════════════════════════════════
       *  PANEL 2 — Chat History (collapsible)
       *  Logo, New Chat, Search, Conversation list
       * ═══════════════════════════════════════════ */}
      <aside
        className={`h-screen bg-[var(--bg-card)] border-r border-[var(--border-color)] flex flex-col justify-between p-4 transition-all duration-250 ease-out overflow-hidden flex-shrink-0 ${
          isSidebarOpen ? 'w-[280px] opacity-100' : 'w-0 p-0 opacity-0'
        }`}
      >
        <div className={`flex flex-col h-full min-w-[248px] ${!isSidebarOpen ? 'invisible' : ''}`}>
          {/* Top — New Chat + Search */}
